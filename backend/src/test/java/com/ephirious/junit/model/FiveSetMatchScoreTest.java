...
public class FiveSetMatchScoreTest {
    private final FiveSetMatchScore match = new FiveSetMatchScore();
    ...
    private static Stream<Arguments> providePointChainForTestFirstScore() {
        return Stream.of(
                ...
                Arguments.of(build("f49"), 2),
                Arguments.of(build("f72"), 3)
        );
    }
    ...
    private static Stream<Arguments> providePointChainForTestSecondScore() {
        return Stream.of(
                ...
                Arguments.of(build("s49"), 2),
                Arguments.of(build("s72"), 3)
        );
    }
    ...
    private static Stream<Arguments> providePointChainWithExpectedWinner() {
        return Stream.of(
                Arguments.of(build("f72"), FIRST),
                Arguments.of(build("s72"), SECOND),
                Arguments.of(build("f24 s24 f24 s24 f24"), FIRST),
                Arguments.of(build("s24 f24 s24 f24 s24"), SECOND),
                Arguments.of(build("f24 s24 f20 s20 f4 s4 f7 f24"), FIRST),
                Arguments.of(build("s24 f24 s20 f20 s4 f4 s7 s24"), SECOND),
                Arguments.of(build("f24 s24 f20 s20 f4 s4 f7 s24 f24 f20 s20 f10"), FIRST),
                Arguments.of(build("s24 f24 s20 f20 s4 f4 s7 f24 s24 f20 s20 f10"), SECOND)
        );
    }
}
