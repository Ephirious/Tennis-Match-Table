...
import com.ephirious.exception.service.MatchWinnerNotMatchPlayersException;
import com.ephirious.exception.service.PlayersPlayingException;
...
public class OngoingMatchService {
    ...
    public Optional<Player> matchWinner(@NonNull Match match, @NonNull Player first, @NonNull Player second) {
        if (match.matchEnded()) {
            ...
            throw new MatchWinnerNotMatchPlayersException("The winner can't resolve");
        }
    ...
    private void ensurePlayerNotPlayingNow(Player player) {
        if (repository.alreadyPlaying(player)) {
            throw new PlayersPlayingException(player.name().value());
        }
    }
}
